/**
 * Yotsuba Society archives, part 2: 4chan API thread dumps under a `4chan/`
 * prefix, 83GB compressed. The prefix is stripped so the converter sees
 * `<board>/<threadno>/`.
 */
...
const src = existsSync('extracted/4chan') ? 'extracted/4chan' : 'extracted';
sh(
