(function (global, factory) {
  ...
  function getProcedure(token) {
    ...
    if (token.type === import_commonjs.SelectorType.Attribute) {
      ...
      if (
        token.action === import_commonjs.AttributeAction.Equals &&
        token.name === 'id'
      )
        proc = 9;
  ...
  function compileUnsafe(selector, options, context) {
    return compileToken(
      typeof selector === 'string'
        ? (0, import_commonjs.parse)(selector)
        : selector,
      options,
      context
    );
  ...
  function includesScopePseudo(t) {
    return (
      t.type === import_commonjs.SelectorType.Pseudo &&
      (t.name === 'scope' ||
  ...
  function absolutize(token, { adapter }, context) {
    ...
    for (const t of token) {
      if (
        t.length > 0 &&
        isTraversal(t[0]) &&
        t[0].type !== import_commonjs.SelectorType.Descendant
      ) {
    ...
    const query = token
      .map((rules) => {
        if (rules.length >= 2) {
          ...
          if (
            first.type !== import_commonjs.SelectorType.Pseudo ||
            first.name !== 'scope'
          ) {
          } else if (
            isArrayContext &&
            second.type === import_commonjs.SelectorType.Descendant
          )
            rules[1] = FLEXIBLE_DESCENDANT_TOKEN;
          else if (
            second.type === import_commonjs.SelectorType.Adjacent ||
            second.type === import_commonjs.SelectorType.Sibling
          )
