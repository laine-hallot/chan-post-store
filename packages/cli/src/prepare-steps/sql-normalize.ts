 ...
 * made Node's readline cut INSERT statements in half. See mysqldump.ts
 * readLines. It also means a dump whose strings carry genuine unescaped
 * newlines (mysqlchump's do) must NOT be routed through here; those sources
 * are already one board per file and need no staging beyond a copy.
 */
...
export interface SqlNormalizeStep {
  /** Directory of input `.sql` files, relative to the dataset dir. */
  from: string;
  /** Directory to write `<board>.sql` into, relative to the dataset dir. */
  to: string;
  /** Which schema the inputs are in. Defaults to 'none'. */
  rename: SqlRename;
}

/**
 * The awk program, as text.
 *
 * Exported so it can be exercised on a fixture without a NAS to run it
 * against -- the renames and the side-table rules are the parts worth testing
 * directly.
 */
...
/**
 * Runs the split on whichever machine holds the archives.
 *
 * Through the runner, never the mount. These inputs run to 21GB in a single
 * file and 275GB across the family; reading them into this process to rewrite
 * them would pull every byte over NFS and push it back. The awk program is
 * written to the dataset dir and run there, so the data never leaves the
 * machine it lives on.
 */
export const runSqlNormalize = async (
  runner: Runner,
  dir: string,
  step: SqlNormalizeStep,
  stepName: string
): Promise<Result<number, Error>> => {
  const scriptPath = `${dir}/.sql-normalize.awk`;
  const w = await runner.writeFile(
    scriptPath,
    Buffer.from(buildSqlNormalizeAwk(step.rename), 'utf8')
  );
  if (w.isErr) {
    return Result.err(
  ...
  const from = `${dir}/${step.from}`;
  const to = `${dir}/${step.to}`;
  // The destination is rebuilt rather than added to: the program appends, so
  // a re-run over a populated directory would double every board's rows.
  const cmd =
    `set -e; rm -rf ${shQuote(to)}; mkdir -p ${shQuote(to)}; ` +
    `found=0; ` +
    `for f in ${shQuote(from)}/*.sql; do ` +
    `  [ -e "$f" ] || continue; ` +
    `  echo "  $f"; ` +
    `  awk -v DEST=${shQuote(to)} -f ${shQuote(scriptPath)} "$f"; ` +
    `  found=$((found+1)); ` +
    `done; ` +
    // A silent zero here would look exactly like a source with nothing to
    // stage, which is the failure mode this codebase keeps being bitten by.
    `if [ "$found" -eq 0 ]; then echo "no .sql files in ${step.from}" >&2; exit 1; fi`;

  // inherit, so a 21GB file does not run for minutes in silence. That means
  // stdout is not captured, so the board count comes from a second call.
  const r = await runner.exec(cmd, { cwd: dir, inherit: true });
  await runner.exec(`rm -f ${shQuote(scriptPath)}`, { cwd: dir });
  if (r.code !== 0) {
    return Result.err(
      new Error(
        `prepare step "${stepName}": splitting failed (exit ${r.code})` +
          (r.stderr.trim() ? `\n  ${r.stderr.trim()}` : '')
      )
    );
  }

  const c = await runner.exec(`ls -1 ${shQuote(to)} | wc -l`, { cwd: dir });
  const boards = Number(c.stdout.trim()) || 0;
  if (boards === 0) {
    return Result.err(
      new Error(
        `prepare step "${stepName}": wrote no board files to ${step.to}\n` +
