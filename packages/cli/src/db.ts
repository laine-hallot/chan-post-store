...
-- what a run would produce -- clear it with \`ingest-all --redo <source>\`.
...
/**
 * Forget that a source was completed, so the next run ingests it again.
 * Silent when the source has never been ingested -- there is no row to clear
 * and nothing has gone wrong.
 */
export const clearSourceCompleted = async (db: Pool, name: string): Promise<void> => {
  await db.query("UPDATE sources SET completed_at = NULL WHERE name = $1", [name]);
};
