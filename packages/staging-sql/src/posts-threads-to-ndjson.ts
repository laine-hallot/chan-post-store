...
export interface PostsThreadsOptions {
  /** The dump to read. */
  from: string;
  /** Where to build `<board>/posts.ndjson`. */
  to: string;
  /** The only site kept; thread rows on any other host are dropped. */
  site?: string;
}

export interface PostsThreadsStats {
  threads: number;
  foreign: number;
  posts: number;
  boards: number;
...
import { NdjsonWriter, readLines, stripHtml } from 'staging-core';
...
import {
  ...
  fromUrl,
  parseDateTime,
} from './posts-threads-dialects.ts';
import {
  CREATE_TABLE_RE,
  insertColumns,
  parseTuples,
...
export const postsThreadsToNdjson = async (
  opts: PostsThreadsOptions
): Promise<PostsThreadsStats> => {
  const { from, to, site: keepSite = '4chan' } = opts;
  ...
  console.log(
    `    indexed ${stats.threads} thread(s), ${stats.foreign} not ${keepSite}`
  );
